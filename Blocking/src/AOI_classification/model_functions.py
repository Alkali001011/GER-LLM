...
def train_BertFE(model, train_x_tensor, train_y_tensor, valid_x_tensor, valid_y_tensor, test_x_tensor, test_y_tensor,
    ...
    for epoch in range(hp.n_epochs):
        ...
        print('\n==========  Validation Epoch:', epoch + 1, ' ==========\n')
        valid_f1 = validate_BertFE(model, valid_x_tensor, valid_y_tensor, b_s, hp.device, 0)

        print('\n==========  Test Epoch:', epoch + 1, ' ==========\n')
        _ = validate_BertFE(model, test_x_tensor, test_y_tensor, b_s, hp.device, epoch + 1)
...
def train_LSTMFE(model, train_x_tensor, train_y_tensor, valid_x_tensor, valid_y_tensor, test_x_tensor, test_y_tensor,
    ...
    for epoch in range(hp.n_epochs):
        ...
        print('\n==========  Validation Epoch:', epoch + 1, ' ==========\n')
        valid_f1 = validate_LSTMFE(model, valid_x_tensor, valid_y_tensor, b_s, hp.device, 0)

        print('\n==========  Test Epoch:', epoch + 1, ' ==========\n')
        _ = validate_LSTMFE(model, test_x_tensor, test_y_tensor, b_s, hp.device, epoch + 1)
...
def validate_BertFE(model, valid_x_tensor, valid_y_tensor, b_s, device, flag):
    attention_mask = np.where(valid_x_tensor != 0, 1, 0)
    attention_mask = torch.tensor(attention_mask)
    ...
    out = 'P: ' + str(round(prec, 4)) + '  |  R: ' + str(round(rec, 4)) + '  |  F1: ' + str(round(f1, 4))
    print(out)

    return f1


def validate_LSTMFE(model, valid_x_tensor, valid_y_tensor, b_s, device, flag):
    model.eval()
    y_pred = np.array([])
    i = 0
    while i < valid_x_tensor.shape[0]:
